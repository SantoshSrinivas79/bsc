...
def get_columns():
	return [
		...
		_("Indicator Name") + ":Data:300",
		_("BSC Target Log") + ":Link/BSC Target Log:100",
		_("Month") + ":Data:40",
		_("Fiscal Year") + ":Data:40",
		_("Target") + ":Float:70",
		_("Achieved") + ":Float:70",
		_("Target Log Percent") + ":Percent:50"	
		]
...
def get_data(filters):
	ini_list = frappe.db.sql("""SELECT log.department, obj.bsc_perspective, 
		ind.bsc_objective, tar.bsc_indicator, ind.full_name, log.name, 
		log.month, log.fiscal_year, log.target, log.achieved, log.target_log_percent
		FROM `tabBSC Target Log` log 
		INNER JOIN `tabBSC Target` tar ON log.bsc_target = tar.name
